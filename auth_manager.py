...
class AuthManager:
    ...
    def create_default_admin(self):
        ...
        if user_count == 0:
            ...
            hashed_password = bcrypt.hashpw("admin".encode(),
            ...
            hashed_password = bcrypt.hashpw("viewer".encode(),
                                            bcrypt.gensalt()).decode()
            self.cursor.execute(
                "INSERT INTO users (username, password, email, role) VALUES (?, ?, ?, ?)",
    ...
    def authenticate_user(self):
        ...
        if st.button("Login"):
            if username and password:
                ...
                if user_record and bcrypt.checkpw(password.encode(),
                                                  user_record[1].encode()):
    ...
    def show_registration_form(self):
        ...
        reg_password = st.text_input("Choose Password",
                                     type="password",
                                     key="reg_password")
        reg_password_confirm = st.text_input("Confirm Password",
                                             type="password",
        ...
        if st.button("Register"):
            ...
            self.cursor.execute("SELECT email FROM users WHERE email = ?",
                                (reg_email, ))
            if self.cursor.fetchone():
            ...
            # Hash password
            hashed_password = bcrypt.hashpw(reg_password.encode(),
                                            bcrypt.gensalt()).decode()
    ...
    def create_session(self, username):
        ...
        session_id = hashlib.sha256(
            f"{username}:{time.time()}".encode()).hexdigest()
    ...
    def logout(self):
        ...
        for key in [
                'authenticated', 'username', 'role', 'current_db',
    ...
    def get_user_role(self, username):
        ...
        self.cursor.execute("SELECT role FROM users WHERE username = ?",
                            (username, ))
